...
namespace ECE141 {
	...
	int ZZPlayer::pcount = 0;
	static PieceColor autoColor1() { //auto assigns a color
		return (++ZZPlayer::pcount % 2) ? PieceColor::blue : PieceColor::gold;
	}
	ZZPlayer::ZZPlayer() : color(autoColor1()) {}


	bool ZZPlayer::isKingable(Game &aGame, const Piece &aPiece, const Location &aDest) {
		if(PieceKind::pawn==aPiece.getKind()) {
			return color == PieceColor::blue ? 0==aDest.row : (aGame.kBoardHeight-1)==aDest.row; //checks if piece can become a king
		}
		return false;
	}

	const Piece* ZZPlayer::isOpponent(Game &aGame, const Location &aLoc, int aDeltaX, int aDeltaY, const Location &before){ // checks if piece is an opponent
		Location near(aLoc.row+aDeltaY,aLoc.col+aDeltaX);
		if (const Tile *theTile=aGame.getTileAt(near)){ //gets the tile
			const Piece* const thePiece=theTile->getPiece(); // gets the piece
			if(thePiece && !thePiece->hasColor(color)) { // if piece exists and color is not the same as current color
...
//				std::cout << "jump: " <<  theJump.row << " " << theJump.col << std::endl;
				...
				if (jumpTile != nullptr) { // can't be jumped
	...
	bool ZZPlayer::oppoNearBy(Game &aGame, const Location &aLoc, const Location &before){
	...
	bool ZZPlayer::capturedLoc(const Location &a, std::vector<Location> locs){
		for (Location b : locs){
	...
	bool ZZPlayer::canJumpTo(Game &aGame, const Location &aLoc, int aDeltaX, int aDeltaY, std::vector<Location> locs) {
	...
	Location* ZZPlayer::pieceHasJump(Game &aGame, const PieceKind pk, const Location &aLoc, PieceColor aColor, std::vector<Location> locs) {
	...
	int ZZPlayer::moveScore(Game &aGame, const Piece &aPiece, int aDeltaX, int aDeltaY) {
		int score = 0;
	...
	int ZZPlayer::moreY(PieceColor aColor) {return PieceColor::blue==aColor ? -1 : 1;}
	int ZZPlayer::lessY(PieceColor aColor) {return PieceColor::blue==aColor ? 1 : -1;}
	int ZZPlayer::moreX(PieceColor aColor) {return PieceColor::blue==aColor ? 1 : -1;}
	int ZZPlayer::lessX(PieceColor aColor) {return PieceColor::blue==aColor ? -1 : 1;}
